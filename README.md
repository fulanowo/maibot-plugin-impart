...
- **反透机制**：长度 < 5cm 时执行透群友有 50% 概率被反透，长度 <= 0 时必被反透
- **白名单系统**：透群友时自动过滤白名单用户
- **CD 限制**：各指令独立冷却时间
...
| `日群友` / `透群友` | 与群友互动（可 @ 指定目标） |
| `日管理` / `透管理` | 与管理互动（需 @ 指定目标） |
| `日群主` / `透群主` | 与群主互动（需 @ 指定目标） |
| `pk` / `对决` | 与 @ 用户进行牛子对决 |
| `打胶` / `开导` | 增加自己的长度 |
| `嗦牛子` / `嗦` | 增加 @ 用户的长度（未 @ 则为自己） |
| `查询` | 查询 @ 用户的长度（未 @ 则为自己） |
| `jj排行榜` / `jjrank` | 展示前 5/后 5 排名及自己的排名（含图表） |
| `注入查询` | 查询 @ 用户被注入量（后接 `历史`/`全部` 查看总历史） |
